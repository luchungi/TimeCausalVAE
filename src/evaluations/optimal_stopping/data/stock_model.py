...
class Model:
    def __init__(self, drift, dividend, volatility, spot, nb_stocks, nb_paths, nb_dates, maturity, name, **keywords):
        self.name = name
    ...
    def generate_paths(self, nb_paths=None):
        ...
        if NB_JOBS_PATH_GEN > 1:
            return np.array(
                joblib.Parallel(n_jobs=NB_JOBS_PATH_GEN, prefer="threads")(joblib.delayed(self.generate_one_path)() for i in range(nb_paths))
            )
        else:
            return np.array([self.generate_one_path() for i in range(nb_paths)]), None


class BlackScholes(Model):
    def __init__(self, drift, volatility, nb_paths, nb_stocks, nb_dates, spot, maturity, dividend=0, **keywords):
    ...
    def generate_paths(self, nb_paths=None, return_dW=False, dW=None, X0=None, nb_dates=None):
        ...
        drift = self.drift
        r = np.repeat(np.repeat(np.repeat(np.reshape(drift, (-1, 1, 1)), nb_paths, axis=0), self.nb_stocks, axis=1), nb_dates, axis=2)
        sig = np.repeat(np.repeat(np.repeat(np.reshape(self.volatility, (-1, 1, 1)), nb_paths, axis=0), self.nb_stocks, axis=1), nb_dates, axis=2)
    ...
    def generate_paths_with_alternatives(self, nb_paths=None, nb_alternatives=1, nb_dates=None):
        ...
        spot_paths[:, :, 0] = self.spot
        random_numbers = np.random.normal(0, 1, (total_nb_paths, self.nb_stocks, nb_dates))
        mult = nb_alternatives * nb_paths
        for i in range(nb_dates - 1):
            random_numbers[nb_paths + i * mult : nb_paths + (i + 1) * mult, :, : nb_dates - i - 1] = np.tile(
                random_numbers[:nb_paths, :, : nb_dates - i - 1], reps=(nb_alternatives, 1, 1)
            )
        dW = random_numbers * np.sqrt(self.dt)
        drift = self.drift
        r = np.repeat(np.repeat(np.repeat(np.reshape(drift, (-1, 1, 1)), total_nb_paths, axis=0), self.nb_stocks, axis=1), nb_dates, axis=2)
        sig = np.repeat(
            np.repeat(np.repeat(np.reshape(self.volatility, (-1, 1, 1)), total_nb_paths, axis=0), self.nb_stocks, axis=1), nb_dates, axis=2
        )
        spot_paths[:, :, 1:] = np.repeat(spot_paths[:, :, 0:1], nb_dates, axis=2) * np.exp(
            np.cumsum(r * self.dt - (sig**2) * self.dt / 2 + sig * dW, axis=2)
        )
...
class FractionalBlackScholes(Model):
    def __init__(self, drift, volatility, hurst, nb_paths, nb_stocks, nb_dates, spot, maturity, dividend=0, **keywords):
    ...
    def generate_one_path(self):
        ...
        for k in range(1, self.nb_dates + 1):
            ...
            path[:, k] = previous_spots + self.drift_fct(previous_spots, (k) * self.dt) * self.dt + np.multiply(diffusion, fracBM_noise[:, k - 1])
        print("path", path)
        return path
...
class Heston(Model):
    ...
    def __init__(
        self, drift, volatility, mean, speed, correlation, nb_stocks, nb_paths, nb_dates, spot, maturity, dividend=0.0, sine_coeff=None, **kwargs
    ):
        ...
        for i in range(self.nb_paths):
            if start_X is None:
                ...
                for k in range(1, self.nb_dates + 1):
                    ...
                    dZ = (self.correlation * normal_numbers_1 + np.sqrt(1 - self.correlation**2) * normal_numbers_2) * np.sqrt(dt)
                    ...
                    var_paths[i, :, k] = (
                        ...
                        * dt
                        + np.multiply(self.var_diffusion_fct(paths[i, :, k - 1], var_paths[i, :, k - 1]), dZ)
                    )

                    paths[i, :, k] = (
                        paths[i, :, k - 1]
                        + self.drift_fct(paths[i, :, k - 1], (k - 1) * dt) * dt
                        + np.multiply(self.diffusion_fct(paths[i, :, k - 1], (k) * dt, var_paths[i, :, k]), dW)
                    )
...
class RoughHeston(Model):
    ...
    def get_frac_var(
        ...
        """
        v0 = vars[0]
        times = (self.dt * step - np.linspace(0, self.dt * (step - 1), step)) ** (self.H - 0.5)
        if len(vars.shape) == 2:
    ...
    def _generate_one_path(self, mu, la, thet, vol, start_X, nb_steps, v0=None):
        ...
        dZ = (self.correlation * normal_numbers_1 + np.sqrt(1 - self.correlation**2) * normal_numbers_2) * np.sqrt(self.dt)
    ...
    def _generate_paths(self, mu, la, thet, vol, start_X, nb_steps, v0=None, nb_stocks=1):
    ...
    def generate_one_path(self):
        ...
        for i in range(self.nb_stocks):
            spot_path, var_path = self._generate_one_path(
                self.drift, self.speed, self.mean, self.volatility, start_X=self.spot, nb_steps=self.nb_dates * self.nb_steps_mult
            )
    ...
    def generate_paths(self, nb_paths=None):
        ...
        spot_paths, var_paths = self._generate_paths(
            ...
            self.volatility,
            start_X=self.spot,
            nb_steps=self.nb_dates * self.nb_steps_mult,
            nb_stocks=self.nb_stocks * nb_paths,
        )
        spot_paths = spot_paths[0 :: self.nb_steps_mult]
        spot_paths = np.reshape(spot_paths, (1 + self.nb_dates, nb_paths, self.nb_stocks))
        spot_paths = np.transpose(spot_paths, axes=(1, 2, 0))
        var_paths = var_paths[0 :: self.nb_steps_mult]
        var_paths = np.reshape(var_paths, (1 + self.nb_dates, nb_paths, self.nb_stocks))
