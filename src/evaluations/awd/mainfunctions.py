import numpy as np
from evaluations.awd.normal_ot import solve_sinkhorn, gurobi_2d, solve_pot, solve_unbalanced
                ...
                for ind_t_y in range(len(y_t_arr)):
                    ...
                    for ind_xp in range(len(x_tp_arr)):
                        ...
                        m.addConstr(
                            ...
                            name="causal_" + str(t) + "_" + str(ind_t) + "_" + str(ind_t_y) + "_" + str(ind_xp),
                        )
            ...
            for ind_t in range(len(x_t_arr)):
                ...
                for ind_t_y in range(len(y_t_arr)):
                    ...
                    for ind_xp in range(len(x_tp_arr)):
                        ...
                        m.addConstr(
                            pi_sum_left * mu_sum_left == pi_sum_right * mu_sum_right,
                            name="anticausal_" + str(t) + "_" + str(ind_t) + "_" + str(ind_t_y) + "_" + str(ind_xp),
                        )
...
def sinkhorn_bicausal_markov(
        ...
        for t_m in range(t_max):
            ...
            if t == 0:
                f_list[0], value_f = update_f_1(
                    ...
                    nu_list[0][1],
                    g_list[0],
                    cost_list[0][mu_list[0][0], :][:, nu_list[0][0]] * cvh[mu_list[0][0], :][:, nu_list[0][0]],
                )
            elif t == t_max - 1:
    ...
    for t_m in range(t_max):
        ...
        if t == t_max - 1:
            ...
            value = np.sum(
                (-np.log(cost_list[0][mu_list[0][0], :][:, nu_list[0][0]]) + V_tp[mu_list[t][0], :][:, nu_list[t][0]])
                * f_list[0]
                * g_list[0]
...
def sinkhorn_causal_markov(
    ...
    def full_update_f_list():
        for t_m in range(t_max):
            ...
            if t == 0:
                f_list[0], value_f = update_f_1(
                    mu_list[0][1], nu_list[0][1], cvh, cost_list[0][mu_list[0][0], :][:, nu_list[0][0]]
                )
            elif t == t_max - 1:
                for i in range(n_list[t - 1]):
                    for j, ind_full in enumerate(nu_index_full[t - 1]):
                        f_list[t][i][j], cvnew[i, j] = update_f_capt(
                            ...
                            g[j],
                            cost_list[t][mu_list[t][i][0], :][:, nu_list[t][ind_full[-2]][0]],
                        )
            else:
                for i in range(n_list[t - 1]):
                    for j, ind_full in enumerate(nu_index_full[t - 1]):
        ...
        for t_m in range(t_max):
            ...
            for i in range(smu_shape):
                if t == t_max - 1:
                    ...
                    Vnext[i, :] = np.sum(
                        -np.log(cost_list[t][mu_list[t][i][0], :][:, nu_list[t][ind_tot[-2]][0]])
                        * cost_list[t][mu_list[t][i][0], :][:, nu_list[t][ind_tot[-2]][0]]
                        * f_list[t][i][ind_tot[-1]]
                        * mu_list[t][i][1],
                        axis=0,
